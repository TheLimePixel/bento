...
class Lexer(private val code: String, private var pos: Int = 0) {
    ...
    val next: GreenEdge
        get() = getAt(pos + current.length)

    private val Int.isIndex get() = this < code.length

    private fun getAt(index: Int): GreenEdge = if (index == code.length) BaseEdges.eof else when (val char = code[index]) {
        '(' -> BaseEdges.lParen
        ')' -> BaseEdges.rParen
        '{' -> BaseEdges.lBrace
        '}' -> BaseEdges.rBrace
    ...
    private tailrec fun getString(curr: Int): GreenEdge = if (!curr.isIndex) finishStringLiteral(curr) else when (code[curr]) {
